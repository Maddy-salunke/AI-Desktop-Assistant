    ...
    with open(CONTACTS_FILE, "r") as file:
        for line_number, line in enumerate(file, start=1):
            if line.strip() == '' or line.strip().startswith('#'):
                continue
            if line.count(',') != 1:
                # More than one comma found in the line
                Speak(f"Error in line {line_number}: Please check the email data file, and make sure only one comma is present in each line.")
                return None
